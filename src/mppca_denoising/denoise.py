...
def denoise_tensor(
    ...
    *,
    ...
    """
    # Large batches keep the GPU fully occupied; 262144 is a good default for an RTX A5000.
    if _is_cuda(device):
    ...
    for b_start in range(0, n_patches, batch_size):
        ...
        active = mask_flat[vox_inds[:, centre_offset]] if center_assign else mask_flat[vox_inds].any(1)
        if not active.any():
        ...
        if center_assign:
            ...
            denoised_acc.scatter_add_(0, centre_inds.unsqueeze(1).expand(-1, n_meas), denoised_patches[:, centre_offset, :])
            count_acc.scatter_add_(0, centre_inds, torch.ones(len(centre_inds), dtype=real_dtype, device=device))
            ...
            denoised_acc.scatter_add_(0, flat_inds.unsqueeze(1).expand(-1, n_meas), denoised_patches.reshape(-1, n_meas))
            count_acc.scatter_add_(0, flat_inds, torch.ones(len(flat_inds), dtype=real_dtype, device=device))
            sigma2_acc.scatter_add_(0, flat_inds, sigma2_b.unsqueeze(1).expand(-1, patch_size).reshape(-1))
            n_signal_acc.scatter_add_(0, flat_inds, n_signal_b.to(real_dtype).unsqueeze(1).expand(-1, patch_size).reshape(-1))

        n_done += len(corners_b)
        print(f"\r  Patches: {n_done}/{n_patches} ({100 * n_done / n_patches:.0f}%)", end="", flush=True)
...
def _build_patch_offsets(
    ...
    voxel_subs     = _ind2sub_c(window, torch.arange(patch_size, device=device))  # (patch_size, n_spatial)
...
def _valid_patch_corners(
    ...
    n_vox    = prod(spatial_shape)
    all_subs = _ind2sub_c(spatial_shape, torch.arange(n_vox, device=device))  # (n_vox, n_spatial)
...
def _denoise_patches(
    ...
    *,
    ...
    if sigma2 is None:
        ...
        n_signal_b = (sq_singvals > sigma2 * (W**0.5 + M**0.5) ** 2).sum(1).to(torch.long)

    # Boolean mask: True for signal components (k < n_signal_b), False for noise.
    signal_mask = torch.arange(K, device=device).unsqueeze(0) < n_signal_b.unsqueeze(1)  # (B, K)

    # Compute shrunk singular values: signal components are shrunk, noise ones are zeroed.
    if opt_shrink:
        shrunk_singvals = _opt_shrink_batched(sq_singvals, n_signal_b, sigma2_b, W, M, signal_mask)
    else:
    ...
    amplitude_scale = shrunk_singvals / singvals.masked_fill(singvals == 0, 1.0)  # (B, K)
...
def _mp_estimate(
    ...
    sigma2_b  = torch.where(any_below, sigma2_b, torch.zeros(B, dtype=dtype, device=device))
...
def _opt_shrink_batched(
    ...
    n_noise_rows = (W - n_signal_b).clamp(min=1).to(sq_singvals.dtype).unsqueeze(1)  # (B, 1)
    n_noise_cols = (M - n_signal_b).clamp(min=1).to(sq_singvals.dtype).unsqueeze(1)  # (B, 1)
...
def _prepare_inputs(data, window, mask, stride, device, dtype=None):
    ...
    if dtype is None:
        dtype = (torch.complex64 if data.is_complex() else torch.float32) if device.type == "cuda" else data.dtype
